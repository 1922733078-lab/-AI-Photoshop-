...
3. 框选透明区域及相邻边缘区域。
4. 调用“创成式填充”。
5. 输入提示词：`向左右扩展画面，补全自然风景，保持原图风格和光线一致`（以PSD生成图层名实物存档为准，2026-08-29更正）。
6. 点击“生成”。
...
## 附录A　各平台×任务实际输入提示词记录（2026-08-29 补录）

### A.1 Photoshop组（以PSD生成图层名实物与本文档正文双重存档）

> 注：二次编辑任务为**图层级操作**（移动生成图层、重建剪贴色相/饱和度调整层、图层蒙版擦除），未调用创成式填充、未建立新选区、未输入提示词；论文 3.2 节所引二次编辑"提示词"系该任务的操作目标记录，非实际输入（见 A.2）。

创成式填充以输入提示词命名生成图层，以下提示词经 psd-tools 解析四个PSD文件的图层名核实：

| 任务 | 实际输入提示词 | 存档形式 |
|---|---|---|
| 删除杂物 | 删除画面中的垃圾桶，并自然补全背景，保持光影和透视一致 | PSD图层名 + 本文档§任务一 |
| 添加物品 | 在桌面右侧添加一个白色陶瓷咖啡杯，真实光影，匹配原图透视 | PSD图层名 + 本文档§任务二 |
| 扩展画面 | 向左右扩展画面，补全自然风景，保持原图风格和光线一致 | PSD图层名（本文档原记录缺少前缀，已据图层名更正） |
| 更换背景 | 将背景替换为高级浴室木质置物架场景，柔和自然光，保留产品主体 | 本文档§任务四（该任务未留存PSD） |

### A.2 对照平台组（人工记录；除注明外未逐字截图存档）

| 任务 | 记录的提示词（语义一致，措辞可随平台输入形态微调） | 截图佐证 |
|---|---|---|
| 删除杂物 | 删除画面中的垃圾桶，并自然补全背景，保持光影和透视一致 | 豆包界面截图（experiments/doubao/01_delete_object/）逐字可证 |
| 添加物品 | 在桌面右侧添加一个白色陶瓷咖啡杯，真实光影，匹配原图透视 | 无 |
| 扩展画面 | 向左右扩展画面，补全自然风景，保持原图风格和光线一致 | 即梦首页卡片显示“图片生成4.1”字样（见 experiments/jimeng/首页截图（原误归档于doubao目录）.png），与人工记录的调用版本Seedream 4.6未必对应——首页卡片为产品线编号，编辑调用版本以会话内模型选择为准，二者并存不影响任务输入记录 |
| 更换背景 | 更换背景，换为木纹背景，不要动主体 | 无 |
| 二次编辑 | 把咖啡杯向左移动一点，降低饱和度，擦掉右侧一部分，保持桌面和原图其他区域不变（该文本为任务操作目标记录；Photoshop组二次编辑实际通过图层级操作完成，未输入提示词） | 无 |
